 ...
 * using the legacy endpoint name <em>control</em>.
 *
 */
public class LegacyCommandClientImpl extends AbstractRequestResponseClient<BufferResult> implements LegacyCommandClient {

    private static final Logger LOG = LoggerFactory.getLogger(LegacyCommandClientImpl.class);
    ...
    protected String getName() {
        return CommandConstants.COMMAND_LEGACY_ENDPOINT;
    }
    ...
    public static final Future<LegacyCommandClient> create(
        ...
        final LegacyCommandClientImpl client = new LegacyCommandClientImpl(con, tenantId, deviceId, replyId);
        return client.createLinks(senderCloseHook, receiverCloseHook)
                .map(ok -> {
                    LOG.debug("successfully created command client for [{}]", tenantId);
                    return (LegacyCommandClient) client;
                }).recover(t -> {
